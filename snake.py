...
class Game():
    ...
    def over(self):
        while 1:
            ...
            txts = pg.font.SysFont('Courier New', 50).render('Game Over    Score:', True,
                                                             (255, 255, 255))
            txtrect = txts.get_rect()
            txtrect.topleft = (20, 150)
            self.screen.blit(txts, txtrect)
            txts = pg.font.SysFont('Courier New', 50).render(str(self.snake.score), True,
                                                             (255, 255, 255))
            txtrect = txts.get_rect()
    ...
    def loop(self):
        ...
        while self.game_over is not True:
            ...
            self.snake.update()
            for x in self.blocks:
                if self.snake.check_collisions(x[1]) is True:
                    self.over()
                self.screen.blit(x[0], x[1])
            a = 0
            for x in self.snake.images:
                if a != 0:
                    if self.snake.check_apple(x[1]) is True:
                        self.over()
                self.screen.blit(x[0], x[1])
                a += 1
            if self.snake.check_apple(self.apple.pos) is True:
...
class Startmenu():
    ...
    def make_button(self, pos, text, color, action=None, textsize=20):
        ...
        rectangle.fill((255, 255, 255, alpha), special_flags=pg.BLEND_RGBA_MIN)
        self.screen.blit(rectangle, pos)
        self.make_text((pos[0] + pos[2] / 2),
                       (pos[1] + pos[3] / 2), text, a=True, size=textsize)
